...
All comparisons store `1` (true) or `0` (false) into an `int` destination.
...
Same mnemonics - operands must both be `float`, `dst` must be `int`.
...
jmpifnot cond, label
```
Jump if `cond` (an `int` register) is non-zero / zero.
...
| `readint`   | `readint   dst` | read a decimal integer; `dst` must be `int` |
...
```asm
add  r0, r0, 1        ; equivalent to: load scratch, 1 / add r0, r0, scratch
lt   r1, r0, 100
shl  r2, r0, 3
```
