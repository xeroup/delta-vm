...
|---|---|---|
| `int` | 64-bit signed integer | counters, indices, boolean results (0/1), bitwise operations |
...
Both source operands must have the same type, and it must be `int` or `float`. The destination must match.

**Float-specific unary/binary (`modf`, `powf`, `negf`, `absf`, `sqrt`):**
All operands must be `float`.

**Bitwise (`and`, `or`, `xor`, `not`, `shl`, `shr`):**
All operands must be `int`. Destination must be `int`.

**Comparisons:**
Source operands must match each other. Destination must be `int`.
...
**Calls:**
Argument count and types are matched against the `.func` or `.extern` declaration. Return type of the callee must match the destination register's type. For variadic externs, at least the fixed parameters must match; extra arguments are unchecked.
...
```asm
add r0, r0, 1        ; fine - 1 loaded into scratch automatically
lt  r1, r0, 100      ; fine
shl r2, r0, 3        ; fine
```
